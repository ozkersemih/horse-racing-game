...
vi.mock('@/features/horse-list/components/HorseListItem.vue', () => ({
  default: {
    name: 'HorseListItem',
    props: ['horse'],
    template: '<div data-testid="stub-horse-item">{{ horse?.name }}</div>',
  },
}))
...
describe('HorseList.vue', () => {
  ...
  it('shows the header text', () => {
    ...
    expect(wrapper.get('[data-testid="horse-list-title"]').text()).toBe('Horse List (1-20)')
  })

  it('renders as many HorseListItem components as there are horses in the store', () => {
    const wrapper = mountWith()
    const items = wrapper.findAll('[data-testid="stub-horse-item"]')
  ...
  it('renders no items when the list is empty', () => {
    ...
    expect(wrapper.findAll('[data-testid="stub-horse-item"]').length).toBe(0)
  })

  it('reflects store changes by remounting (non-reactive mock)', () => {
    let wrapper = mountWith()
    expect(wrapper.findAll('[data-testid="stub-horse-item"]').length).toBe(3)
